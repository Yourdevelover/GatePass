...
This migration creates the complete database schema for the GatePass parking payment application.

1. New Tables
- `profiles` - Extended user data (name, phone, balance, isAdmin) linked to auth.users
- `vehicles` - User's registered vehicles (motor, mobil, truk)
- `parking_locations` - Parking locations with rates and slot availability
- `parking_sessions` - Active and completed parking sessions
- `transactions` - Payment transactions for parking
- `topups` - Balance top-up history

2. Security (Row Level Security)
- All tables have RLS enabled
- Owner-scoped policies for user data (vehicles, parking_sessions, transactions, topups)
- Public read for parking_locations (all users can view available parking)
- Admin-only write access for parking_locations
- Profiles table with user access to own data

3. Default Data
- Inserts 5 default parking locations in Jakarta
...
DROP POLICY IF EXISTS "Anyone can read parking locations" ON parking_locations;
CREATE POLICY "Anyone can read parking locations" ON parking_locations FOR SELECT
  TO anon, authenticated USING (true);

DROP POLICY IF EXISTS "Admins can insert parking locations" ON parking_locations;
CREATE POLICY "Admins can insert parking locations" ON parking_locations FOR INSERT
  TO authenticated WITH CHECK (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = true)
  );

DROP POLICY IF EXISTS "Admins can update parking locations" ON parking_locations;
...
DROP POLICY IF EXISTS "Users can read own parking sessions" ON parking_sessions;
CREATE POLICY "Users can read own parking sessions" ON parking_sessions FOR SELECT
  TO authenticated USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own parking sessions" ON parking_sessions;
CREATE POLICY "Users can insert own parking sessions" ON parking_sessions FOR INSERT
  TO authenticated WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own parking sessions" ON parking_sessions;
...
-- Topups policies
DROP POLICY IF EXISTS "Users can read own topups" ON topups;
